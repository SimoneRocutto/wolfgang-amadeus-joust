...
joinBtn.addEventListener('click', () => {
    ...
    // SBLOCCO AUDIO: Sui browser mobile l'audio deve essere "toccato" una volta dall'utente
...
function handleMotion(event) {
    ...
    // Invio dati al server (puoi rimettere il limite di tempo se vuoi)
    socket.emit('motion_data', { intensity: intensity });
}

// RICEZIONE MORTE
socket.on('game_over', () => {
    if (!isAlive) return; // Evita di suonare mille volte

    isAlive = false;
    document.body.className = 'dead';
    statusDiv.innerText = "SEI MORTO!";

    // Suona il vetro rotto
    deathSound.play();

    // Prova vibrazione (su alcuni Firefox Android è supportata)
